...
export default function MyFilesSection() {
  ...
  const [menuOpen, setMenuOpen] = useState(null);          // {folderId, fileId} | null
  const [renaming, setRenaming] = useState(null);          // {folderId, fileId} | null
  const [renameVal, setRenameVal] = useState("");

  const [deleteTarget, setDeleteTarget] = useState(null);  // {folderId, fileId} | null
      ...
      {filtered.length === 0 ? (
        ...
        filtered.map((fo) => (
          <div key={fo.loanId} className="border rounded-lg shadow-sm">
            ...
            >
              <span className="flex items-center gap-3 text-lg font-semibold text-blue-700 cursor-pointer">
                <FiFolder className="text-yellow-500" />
                {fo.loanName} ({fo.loanId})
              </span>
              {expanded[fo.loanId] ? (
                          ...
                          >
                            <li>
                              <button
                                ...
                                className="flex w-full items-center gap-2 px-4 py-2 text-left hover:bg-gray-100 cursor-pointer"
                              >
                                <FaEdit /> Rename
                              </button>
                            </li>
